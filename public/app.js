...
function minorNotes(list, r) {
  const items = list.map((f) => {
    ...
    const where = lines.length
      ? `<div class="mn-block"><span class="mn-k">Where</span><ul class="mn-where" data-no-glossary>${lines.slice(0, 5).map((l) => `<li>${linkifyLine(l, r)}</li>`).join("")}</ul></div>`
      : "";

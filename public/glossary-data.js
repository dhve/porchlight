...
export const glossary = [
  ...
  term('runtime-error', 'runtime error', 'A problem that occurs while page code is running. It can interrupt a feature, but the error alone does not identify a broken button or prove that visitors noticed a problem.', ['runtime errors']),
  term('hydration', 'hydration', 'The step where page code connects interactive behavior to HTML that the server already sent. A browser can sometimes recover from a mismatch, so a hydration error alone does not prove the page stopped working.', ['hydration error', 'hydration errors']),
  term('compiled-bundle', 'compiled bundle', 'A file containing website code that has been combined or transformed for the browser. Its line numbers may differ from the original files a developer edits.', ['bundle']),
  term('stack-trace', 'stack trace', 'A record of the code locations involved when an error occurred. It can help a developer trace the cause, but does not directly identify a visible element on the page.', ['stack traces']),
  term('dom', 'DOM', 'The browser’s working representation of a page’s elements, such as headings, buttons, and images. Scripts can change it after the original HTML loads.'),
